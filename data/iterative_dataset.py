...
class IterativeDataset(Dataset):
    def __init__(self, episodes, cfg, normalizer):
        ...
        # Bounded exception for this diagnosed dataset:
        # at most 3 episodes, and at most 1% of all episodes.
        if (
            len(rejected) > 3
            or len(rejected) / max(len(episodes), 1) > 0.01
        ):
            raise ValueError(
        ...
        for index, excess in rejected:
            print(
                f'[IterativeDataset] Excluding episode index={index}, '
        ...
        print(
            f'[IterativeDataset] Using {len(kept)}/{len(episodes)} episodes, '
