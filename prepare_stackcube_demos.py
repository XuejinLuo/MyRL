"""Download and replay official StackCube demos; write the resolved H5 path."""
import argparse
import json
import subprocess
import sys
...
def main():
    ...
    if args.raw:
        ...
        subprocess.run([sys.executable, '-m', 'mani_skill.utils.download_demo',
                        'StackCube-v1', '-o', str(out)], check=True)
        candidates = []
        for path in out.rglob('*.h5'):
            ...
            if (meta.get('env_info', {}).get('env_id') == 'StackCube-v1'
    ...
    if meta['env_info']['env_id'] != 'StackCube-v1':
        raise ValueError('Raw data is not StackCube-v1')
    ...
    subprocess.run([sys.executable, '-m', 'mani_skill.trajectory.replay_trajectory',
        ...
        '--target-control-mode', 'pd_ee_delta_pose', '--sim-backend', 'physx_cpu',
        '--save-traj', '--count', str(args.count), '--num-procs', '1'], check=True)
    files = list(raw.parent.glob('*.pointcloud.pd_ee_delta_pose.physx_cpu.h5'))
    if len(files) != 1:
    ...
    (out/'stackcube_h5_path.txt').write_text(str(files[0]) + '\n')
