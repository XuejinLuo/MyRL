...
def run_iterations(cfg, wandb_run=None):
    ...
    for round_index in range(state['next_round'], cfg.rounds):
        ...
        baseline = evaluate()
        save(rd/'checkpoints'/'epoch_0000.pth', 0, baseline)
        save(rd/'checkpoints'/'best.pth', 0, baseline)
        write_selection(rd, 0, baseline, rd/'checkpoints'/'best.pth', stage='iterative', round=rd.name)
        for destination in (rd, output):
            log_metrics(destination, 0, baseline, stage='iterative', round=rd.name, sampler=cfg.eval.sampler,
                        checkpoint=str(rd/'checkpoints'/'epoch_0000.pth'),
        ...
        selected = train_round(cfg, base, normalizer, episodes, rd, evaluate, save, baseline,
            log_callback=(lambda metrics, epoch: wandb_run.log(metrics,
                step=round_index*(cfg.epochs+1)+epoch)) if wandb_run else None)
        state['checkpoint'] = selected or str(rd/'checkpoints'/'epoch_0000.pth')
